...
const categoryConfig: Record<ComponentCategory, { label: string; icon: any; description: string }> = {
  'grow-tent': { label: 'Tents', icon: Home, description: 'Closed environments' },
  'led-light': { label: 'Lights', icon: Lightbulb, description: 'Full spectrum grow lights' },
  'ventilation': { label: 'Ventilation', icon: Wind, description: 'Fans and air circulation' },
  'carbon-filter': { label: 'Carbon Filters', icon: Filter, description: 'Odor control systems' },
  'ph-meter': { label: 'pH Meters', icon: Beaker, description: 'Testing and monitoring' },
  'nutrients': { label: 'Nutrients', icon: DropletIcon, description: 'Plant feeding solutions' },
  'accessories': { label: 'Accessories', icon: Settings, description: 'Timers, tools & extras' },
...
export default function CategoryNavigation({
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between gap-2 overflow-x-auto px-2 py-3 lg:hidden">
        {Object.entries(categoryConfig)
          .filter(([category]) => componentCounts[category as ComponentCategory] > 0)
          .map(([category, config]) => {
            const Icon = config.icon;
            ...
            return (
              <button
                key={category}
                onClick={() => onCategoryChange(category as ComponentCategory)}
                className={`relative flex flex-col items-center flex-shrink-0 px-2 ${
              ...
              >
                ...
                {count > 0 && (
                  <span className="absolute top-0 right-1 bg-primary text-white text-[10px] rounded-full px-1">
                    {count}
                  </span>
                )}
              </button>
